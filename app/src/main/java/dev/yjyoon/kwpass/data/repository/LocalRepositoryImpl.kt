...
import dev.yjyoon.kwpass.data.exception.NonAccountDataException
...
class LocalRepositoryImpl @Inject constructor(
    ...
    override suspend fun setUserData(user: User): Result<Unit> = runCatching {
        dataStore.edit {
            it[stringPreferencesKey(KEY_USER_ID)] = user.id
            it[stringPreferencesKey(KEY_USER_PW)] = user.password
            it[stringPreferencesKey(KEY_USER_PHONE)] = user.phone
        }
    }
